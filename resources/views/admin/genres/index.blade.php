<x-app-layout>
    <div class="max-w-6xl mx-auto px-6 py-8">
        ...
        <nav class="flex items-center text-sm text-text-muted mb-6">
            ...
            <span class="text-text-primary font-medium">Genres</span>
        </nav>
        ...
        <div class="mb-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <form method="GET" action="{{ route('admin.genres.index') }}" class="flex w-full md:w-auto items-center gap-2">
                    ...
                    <table class="w-full text-sm border-separate border-spacing-y-2">
                        <thead class="sticky top-0 bg-navbar/70 backdrop-blur border-b border-surface/70">
                            <tr class="text-left text-text-muted">
                                <th class="px-4 py-2 font-medium">Genre</th>
                            ...
                            @foreach ($genres as $g)
                                <tr class="rounded-lg bg-surface/10 hover:bg-surface/25 transition">
                                    <td class="px-4 py-3">
                                        ...
                                        <div class="text-[11px] text-text-muted">ID: {{ $g->id }}</div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="inline-flex items-center gap-2">
                                            <a href="{{ route('admin.genres.edit', $g) }}"
                                            ...
                                            <form action="{{ route('admin.genres.destroy', $g) }}" method="POST"
                                                  onsubmit="return confirm('Weet je zeker dat je dit genre wilt verwijderen?');">
                                                @csrf
                                                @method('DELETE')
